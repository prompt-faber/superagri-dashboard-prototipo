...
# ====================== UPLOAD DE CSV ======================
st.sidebar.header("📤 Inserção de Dados Reais")
uploaded_file = st.sidebar.file_uploader("Carregue seu CSV exportado do Bling", type=["csv"])

if uploaded_file is not None:
    df = pd.read_csv(uploaded_file)
    st.sidebar.success("✅ CSV carregado com sucesso!")
else:
    # Dados simulados (caso não suba CSV)
    data = {
...
with tab1:
    ...
    with col_g1:
        ...
        fig_bar = px.bar(df_filtrado, x="produto", y="estoque_atual", color="categoria", title="Nível de Estoque")
...
with tab3:
    st.subheader("📈 Vendas vs Meta Mensal")
...
with tab5:
    st.subheader("🔥 Recomendações Inteligentes de Upsell")
    st.success("Pulverizador Elétrico 20L + Kit Mangueira 50m → vendido junto em 68% das vendas")
    st.success("Bomba 12V + Inoculante 5L → combo mais vendido para pequenos produtores")
    st.success("Lavadora 2000 PSI + Pulverizador Costal → pacote completo")

st.caption(f"Dashboard gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')} | Upload de CSV ativado")
